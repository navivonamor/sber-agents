...
class ChatBot:
    ...
    def add_message(self, role: str, content: str):
        ...
        if len(self.conversation_history) > MAX_MESSAGES:
            # Находим сообщение системного промпта
            system_messages = [msg for msg in self.conversation_history \
                if msg["role"] == "system"][-1:]
            # Берем последние сообщения (исключая системный промпт)
            recent_messages = [msg for msg in self.conversation_history \
                if msg["role"] != "system"][-MAX_MESSAGES:]
            # Объединяем системные сообщения с последними MAX_MESSAGES сообщениями
