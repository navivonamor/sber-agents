...
- ✅ Транскрибация голосовых сообщений (Faster-Whisper на CPU)
...
- Faster-Whisper работает на CPU (для GPU требуется CUDA)
...
- `WHISPER_MODEL` - модель Faster-Whisper (tiny, base, small, medium, large), по умолчанию "base"
- `WHISPER_DEVICE` - устройство для Whisper (cpu или cuda), по умолчанию "cpu"
- `WHISPER_COMPUTE_TYPE` - тип вычислений (int8, float16, float32), по умолчанию "int8"

**Настройка Faster-Whisper:**

Для работы на CPU (по умолчанию):
- Используйте `WHISPER_MODEL=base` (баланс скорости и качества)
- Используйте `WHISPER_DEVICE=cpu`
- Используйте `WHISPER_COMPUTE_TYPE=int8` (оптимально для CPU)

Для работы на GPU (требуется CUDA):
- Установите `WHISPER_MODEL=small` или `medium`
- Установите `WHISPER_DEVICE=cuda`
- Установите `WHISPER_COMPUTE_TYPE=float16`
...
```
Пользователь: [отправляет голосовое сообщение: "Купил продукты на полторы тысячи рублей"]
Бот: 🎤 Распознано: "Купил продукты на полторы тысячи рублей"

Записал ваш расход на продукты в размере 1500 рублей.
...
- Транскрибация голосовых сообщений работает на CPU (для GPU требуется CUDA и изменение настроек)
